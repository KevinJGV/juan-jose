...
def AgregarEmpleado(Dicc):
    ...
    if Id in Dicc:
        ...
        with open("emplacme.dat", "a") as OpenFile:
            OpenFile.write("{0};{1};{2};{3}\n".format(Id, Name, Horas, ValorHora))
...
def UpdateFile(Id, Dicc):
    with open("emplacme.dat", "w") as OpenFile:
        OpenFile.write("ID;NOMBRE;HORASTRAB;VALHORA\n")
        for Id in Dicc:
            OpenFile.write("{0};{1};{2};{3}\n".format(Id, Dicc[Id]["Nombre"], Dicc[Id]["Hours"], Dicc[Id]["Value"]))
             
#Main Code
DatosEmpleados = {}
try:
    OpenFile = open("emplacme.dat")
    Contenido = OpenFile.readlines()
    for i in range(1, len(Contenido)):
       List = Contenido[i].split(";")
       DatosEmpleados[int(List[0])] = {}
       DatosEmpleados[int(List[0])]["Nombre"] = List[1]
       DatosEmpleados[int(List[0])]["Hours"] = List[2]
       DatosEmpleados[int(List[0])]["Value"] = List[3]   
    OpenFile.close()
except FileNotFoundError:
    OpenFile = open("emplacme.dat", "w+")
    OpenFile.write("ID;NOMBRE;HORASTRAB;VALHORA\n")
    OpenFile.close()
